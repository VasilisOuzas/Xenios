...
Current versions of Xenios do not implement any networking mechanisms between multiple devices. That means that all of the modified data regarding rooms and reservations are stored and can be processed by one single device. Since the application does not support interconnection between devices, I have intentionally chosen to store all data in simple and easy to maintain and share .json files. Therefore, if you want to manage the same rooms/reservations on a different device, you would only need to copy the needed /data - /reservations folders from the one device to another. 

## Linux users
For the time being, I am not going to include any linux files in the binaries. As a linux user myself, I believe that downloading and running a .py is a no brainer. Althouth, I decided to create an [.sh file](https://github.com/VasilisOuzas/Xenios/blob/main/src/Xenios.sh) that helps the users download automatically all the dependencies and the Tkinter libraries. This is what your diractery must look like to run Xenios:
<div align="left">
  <div>
...
And simply run the following command inside the directory:
```
./Xenios.sh
```
